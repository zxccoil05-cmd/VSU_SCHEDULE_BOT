...
class FSPIPParser:
    ...
    async def _find_all_links(self):
        """Ищем все файлы дневной формы, игнорируя лишнее"""
        try:
            ...
            found_urls = []
            # Черный список для отсева заочки и зачетов
    ...
    async def refresh(self):
        ...
        if not links:
            logger.error("❌ Файлы расписания ФСПиП не найдены!")
            return None
        ...
        for url in links:
            try:
                ...
                # Поиск строки с группами (сканируем 12-15)
                subgroups = {}
                group_row = 14
                ...
                logger.error(f"⚠️ Ошибка в файле ФСПиП {url}: {e}")

        self.cache = all_fac_data
        logger.info(f"✅ ФСПиП обновлен. Групп всего: {len(all_fac_data)}")
        return all_fac_data
