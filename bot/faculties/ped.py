...
class PedParser:
    ...
    async def _find_all_links(self):
        """Ищем только нужные файлы, игнорируя заочку и зачеты"""
        try:
            ...
            found_urls = []
            # Список стоп-слов, которые НЕ должны встречаться в ссылке или тексте
            black_list = ["заоч", "зфо", "зфпо", "зачет", "экзамен", "сессия"]

            for l in links:
                text = l.get_text(separator=" ", strip=True).lower()
                href = l['href'].lower()
                
                # Проверяем, что это расписание и файл Excel
                if "расписание" in text and (".xlsx" in href or ".xls" in href):
                    # Проверяем, нет ли в названии слов из черного списка
            ...
            logger.info(f"🔎 Найдено подходящих файлов ПФ: {len(unique_urls)}")
            return unique_urls
    ...
    async def refresh(self):
        ...
        if not links:
            logger.error("❌ Подходящие файлы расписания ПФ не найдены!")
            return None
        ...
        for url in links:
            try:
                logger.info(f"📥 Обработка дневной формы ПФ: {url}")
                response = requests.get(url)
                wb = openpyxl.load_workbook(BytesIO(response.content), data_only=True)
                sheet = wb.active
                
                # Поиск строки с группами
                subgroups = {}
                group_row = 14
                for r_check in [13, 14, 15, 12]:
                ...
                logger.error(f"⚠️ Ошибка в файле {url}: {e}")

        self.cache = all_fac_data
        logger.info(f"✅ ПФ обновлен. Всего групп дневной формы: {len(all_fac_data)}")
        return all_fac_data
