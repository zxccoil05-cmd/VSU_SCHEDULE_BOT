...
class LawParser:
    def __init__(self):
        # URL страницы расписания Юридического факультета
        self.base_page_url = "https://vsu.by/universitet/fakultety/yuridicheskij/raspisanie.html"
        self.cache = None
    ...
    async def _find_actual_link(self):
        try:
            ...
            for l in links:
                ...
                link_href = l['href']
                # Ищем "Расписание", "дневная" и ".xlsx"
                if "Расписание" in link_text and (".xlsx" in link_href or ".xls" in link_href):
                    if "заоч" not in link_text.lower() and "экзам" not in link_text.lower():
                        return urljoin(self.base_page_url, link_href)
            return None
        except Exception as e:
            logger.error(f"Ошибка поиска ссылки ЮФ: {e}")
            return None
    ...
    async def refresh(self):
        ...
        try:
            logger.info(f"Скачивание файла ЮФ: {actual_url}")
            response = requests.get(actual_url)
            wb = openpyxl.load_workbook(BytesIO(response.content), data_only=True)
            sheet = wb.active
            
            subgroups = {}
            # У юристов группы обычно тоже в 14-й строке, проверяем
            ...
            logger.info("✅ ЮФ успешно обновлен")
            return final_data
        except Exception as e:
            logger.error(f"Ошибка парсинга Excel ЮФ: {e}")
            return None
