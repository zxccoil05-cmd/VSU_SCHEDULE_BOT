...
class FMiITParser:
    def __init__(self):
        # Встроенная ссылка для ФМиИТ
        self.base_page_url = "https://vsu.by/universitet/fakultety/matematiki-i-it/raspisanie.html"
        self.cache = None

    def _get_value(self, sheet, row, col):
        """Твоя изначальная функция для пробития объединенных ячеек"""
    ...
    async def _find_actual_link(self):
        ...
        try:
            ...
            logger.error(f"Ошибка поиска ссылки: {e}")
            return None

    async def refresh(self):
        """Твой основной метод парсинга (get_schedule), переименованный в refresh"""
        actual_url = await self._find_actual_link()
        if not actual_url:
            return None

        try:
            logger.info(f"Скачивание файла ФМиИТ: {actual_url}")
            response = requests.get(actual_url)
            wb = openpyxl.load_workbook(BytesIO(response.content), data_only=True)
            sheet = wb.active
            
            subgroups = {}
            # Группы в 14-й строке
            ...
            logger.info("✅ ФМиИТ успешно обновлен")
            return final_data
        except Exception as e:
            logger.error(f"Ошибка парсинга Excel ФМиИТ: {e}")
            return None
